...
#include <algorithm> // Transform function for Upper Case
...
void playHangman(const string& word, const string& hint)
{
	...
	while (remainingAttempts > 0 && maskedWord != word)
    {
        ...
        // If the guessed letter is found in the string guessedLetters,execute the following code block
        {
        	system("cls");
            remainingAttempts--;
            cout << "You have already guessed that letter :D \n\nRemaining attempts: " << remainingAttempts << endl<<endl;
            continue;
		}
    ...
    if (maskedWord == word)
    {
        cout << "CONGRATULATIONS! You guessed the word ^_^ : " << word << endl<<endl;
    }
    else
    {
...
int main()
{
    ...
    // convert the string to upper case
    transform(hint.begin(), hint.end(), hint.begin(), ::toupper);    
